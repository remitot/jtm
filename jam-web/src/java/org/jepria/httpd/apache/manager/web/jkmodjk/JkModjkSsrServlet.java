...
public class JkModjkSsrServlet extends SsrServletBase {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    if (checkAuth(req)) {
      ...
      JkTextPageContent content = new JkTextPageContent(context, modJkConfLines, itemModRequestLines, CurrentMenuItem.JK_MODJK);
    ...
    if ("/mod".equals(path)) {
      ...
      if (itemModRequests != null && itemModRequests.size() > 0) {
        ...
        if (checkAuth(req)) {
          ...
          if (lines != null) {
            api.updateMod_jk_Conf(lines, apacheConf);
